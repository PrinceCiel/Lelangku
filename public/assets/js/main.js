...
(function () {
    ...
    window.triggerTableLoader = function (callback) {
        const loader = $("#table-loader");
        const tableArea = $(".card-datatable");

        if (loader.length) {
            // 1. Munculkan loader dengan paksa gaya FLEX
            loader.css("opacity", 1).fadeIn({
                duration: 200,
                start: function () {
                    $(this).css("display", "flex"); // Paksa display flex biar loader di tengah
                },
            });

            // 2. Efek blur pada tabel
            tableArea.css({
                filter: "blur(4px)", // Naikkan dikit blurnya biar dramatis
                transition: "all 0.3s ease",
            });

            // 3. Jeda biar user lihat animasinya (naikkan ke 1.2 detik biar puas liat loadernya)
            setTimeout(() => {
                // 4. Hilangkan loader
                loader.fadeOut(300, function () {
                    tableArea.css("filter", "none");

                    // 5. Jalankan perintah selanjutnya (buka modal)
